...
def main():
    ...
    st.write("You may have noticed that when you play with datapack that add biomes e.g. Terralith, that mods like Xaero's minimap, Journeymap, MiniHUD and other mods that read biome names have untranslated strings, often looking something like 'biome.namespace.biomename', which is not all that pretty. This tool fixes that by generating a lang file that you can use in a reourcepack. If you wish to fix multiple packs, you can combine them at https://weld.smithed.dev/ and then submit the combined pack back into here.")
